...
class Solution {
    public List<List<String>> accountsMerge(List<List<String>> accounts) {
        ...
        for(List<String> account:accounts){
            ...
            int first_id=email_to_id.get(first_mail);

            for(int i=2;i<account.size();i++){
                String snd_mail=account.get(i);

                dsu.union(first_id,email_to_id.get(snd_mail));
            }
        }

        HashMap<Integer,List<String>> groups =new HashMap<>();

        for(String emails:email_to_id.keySet()){
           int root=dsu.find(email_to_id.get(emails));

           groups.computeIfAbsent(root,k->new ArrayList<>()).add(emails);
        }
